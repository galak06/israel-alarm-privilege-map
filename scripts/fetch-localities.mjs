 ...
 *   4. Fetch 30-day alert totals per city from Redalert (CORS-compatible, requires API key)
 *   5. Write public/localities.json
 *
...
// ── 4. Shelter distribution estimate (from migun_time) ───────────────────────
...
// ── 6. Redalert 30-day history (/stats/history) ───────────────────────────────
// Paginates all real-alarm events in the last 30 days.
// Produces per-city: alarmCount (real), notifCount (newsFlash), minGapHours.
// CORS-compatible (Access-Control-Allow-Origin: *). Requires API key.
...
async function fetchRedalertHistory30d(apiKey) {
  const endDate   = new Date().toISOString();
  const startDate = new Date(Date.now() - 30 * 24 * 3600 * 1000).toISOString();
  console.log(`📡 Fetching 30d history from Redalert (/stats/history)…`);
  console.log(`   📅 ${startDate.slice(0, 10)} → ${endDate.slice(0, 10)}`);

  // cityData: name → { alarms, notifs, timestamps[] }
  const cityData = {};
  const LIMIT = 100;
  let offset = 0;
  let hasMore = true;
  ...
  try {
    while (hasMore) {
      const url = `https://redalert.orielhaim.com/api/stats/history?startDate=${startDate}&endDate=${endDate}&limit=${LIMIT}&offset=${offset}`;
      const res = await fetch(url, {
      ...
      for (const record of records) {
        ...
        // Capture cityZone while we're here
        for (const city of record.cities ?? []) {
          const name = city.name;
          if (!name) continue;
          if (city.zone && !cityZoneMap[name]) cityZoneMap[name] = city.zone;
          if (!cityData[name]) cityData[name] = { alarms: 0, notifs: 0, timestamps: [] };
          if (isReal) {
            cityData[name].alarms++;
            cityData[name].timestamps.push(record.timestamp);
          } else {
            cityData[name].notifs++;
          }
        }
      }
      ...
      pageNum++;
      process.stdout.write(`\r   Page ${pageNum}: ${offset} records, ${Object.keys(cityData).length} cities`);
      await new Promise(r => setTimeout(r, 200));
    }
    process.stdout.write('\n');
  } catch (e) {
    console.warn(`   ⚠ redalert history failed: ${e.message}`);
  }

  // Compute minGapHours per city from sorted timestamps
  const result = {};
  for (const [name, data] of Object.entries(cityData)) {
    let minGapHours = null;
    if (data.timestamps.length >= 2) {
      const sorted = [...data.timestamps].sort();
      let min = Infinity;
      for (let i = 1; i < sorted.length; i++) {
        const gap = (new Date(sorted[i]) - new Date(sorted[i - 1])) / 3_600_000;
        if (gap < min) min = gap;
      }
      minGapHours = Math.round(min * 100) / 100;
    }
    result[name] = { alarms: data.alarms, notifs: data.notifs, minGapHours };
  }

  const totalAlarms = Object.values(result).reduce((s, d) => s + d.alarms, 0);
  console.log(`   ✓ ${Object.keys(result).length} cities | ${totalAlarms} real alarms | ${pageNum} pages`);
  return result;
}
...
async function main() {
  ...
  // Fetch Oref locality list (city names + migun_time)
  // If blocked (403, e.g. GitHub Actions without local IP), abort — run locally.
  let orefLocalities;
  try {
    orefLocalities = await fetchOrefLocalities();
  } catch (e) {
    console.error(`❌ Cannot fetch Oref locality list: ${e.message}`);
    console.error('   Run this script locally — Oref blocks non-Israeli IPs.');
    process.exit(1);
  }

  // Fetch Redalert 30-day totals for alert counts + zone data
  const apiKey = process.env.REDALERT_API_KEY;
  let redalertTotals = {};
  if (apiKey) {
    redalertTotals = await fetchRedalertHistory30d(apiKey);
  } else {
    console.warn('⚠  REDALERT_API_KEY not set — alertCountTotal will be 0 for all cities.');
  }
  ...
  // Name matching: exact match first, then substring (≥5 chars, ≥70% overlap)
  ...
  for (let i = 0; i < orefLocalities.length; i++) {
    ...
    const history     = redalertKey ? redalertTotals[redalertKey] : null;
  ...
  // Normalize alertCountNormalized across all cities
  ...
  console.log(`   Redalert max 30d: ${maxTotal}`);
